...
export function IdentitiesManager({
  ...
  const { refetch, data, isError } = useQuery({
    ...
    queryFn: () => {
      const serializedIdentities =
        localStorage.getItem(identityListLocalStorageKey) ?? "[]";
  ...
  const addIdentity = useMutation({
    mutationFn: async (newIdentity: Identity) => {
      ...
      localStorage.setItem(
        identityListLocalStorageKey,
        JSON.stringify(newIdentities),
      );
      refetch();
  ...
  return (
    <Flex direction="column" gap="size-200">
      {isError ? <AlertNotification msg="Problem loading identities" /> : null}
      <CreateNewIdentityForm onSubmit={addIdentity.mutate} />
      <IdentitiesList
        identities={data ?? []}
        onDelete={(id) => delIdentity.mutate(id)}
