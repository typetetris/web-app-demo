...
export class ChatClient {
    ...
    constructor(chatId: string, userId: string, displayName: string) {
        ...
        fetch(`${ENDPOINT}/history/${chatId}`).then(async (response) => {
            ...
            if (response.status != 404 && !response.ok) {
                ...
                throw new Error(`error fetching history: ${response.status} ${response.statusText} ${errorBody}`)
            }
            if (response.ok) {
                const body = await response.text();
                const wireMessages = (JSON.parse(body) as RawChatMessage[]).map(convertChatMessageFromWire);
    ...
    private onHistory = (history: ChatMessage[]) => {
        ...
        this.messages = [...combinedMessages.values()].sort((a: ChatMessage, b: ChatMessage) => b.timestamp.getDate() - a.timestamp.getDate());
        this.dispatchSnapshotChange();
    }
